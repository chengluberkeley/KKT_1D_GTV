...
void l2l1nwProfile(int rounds, const std::string& path) {
    ...
    std::vector<gen_data_type> inputSizeDataType = {MPO_NW_INPUT_SIZE, CONDAT_WORST_CASE};
        ...
        for (int i = 0; i < numScales; ++i) {
            ...
            for (int iter = 0; iter < rounds; ++iter) {
                ...
                switch (csvData._genDataType) {
                    case KKT_LP_LQ: {
                        genLpLqFuncs(n, 0, 1, &inputData, true);
                        inputData._lb = -1;
                        inputData._ub = 1;
                        break;
                    }
    ...
    std::vector<gen_data_type> lambdaDataType = {MPO_NW_LAMBDA};
        ...
        for (int i = 0; i < numScales; ++i) {
            ...
            for (int iter = 0; iter < rounds; ++iter) {
                ...
                switch (csvData._genDataType) {
                    ...
                    default: {
                        genLpLqFuncs(n, &inputData, csvData._genDataType, in_lambda);
                        inputData._lb = -2;
                        inputData._ub = 2;
                    }
                }
